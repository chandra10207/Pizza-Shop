...
def create_order_screen():
    ...
    toppings = []
    tk.Label(win, text='Customize your pizza '+str(len(pizza_list)+1), font=('Arial',32)).grid(row=0, column=0,columnspan=2,sticky="nsew",padx=30,pady=30)
    
    tk.Label(win, text='Select pizza '+str(len(pizza_list)+1)+' size', font=('Arial',16)).grid(row=1, column=0)        
    global pizza_size_var
    pizza_size_var = tk.StringVar()
    pizza_size_var.set(pizza_sizes[0]) 
    OptionMenu(win, pizza_size_var, *pizza_sizes).grid(row=1, column=1, sticky="nw",pady=30)
